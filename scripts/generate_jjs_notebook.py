# -*- coding: utf-8 -*-
"""
Generate JJS_analysis.ipynb — a publication-ready Jupyter Notebook template
for AFM force-distance analysis of suspended COF films.
"""
import json
import base64
...
OUT_PATH = ROOT / "reports" / "JJS_analysis.ipynb"
...
def code_cell(source: str):
    ...
    if lines and lines[-1] == "\n":
        lines.pop()
    return {"cell_type": "code", "execution_count": None, "metadata": {}, "outputs": [], "source": lines, "id": str(uuid.uuid4())[:8]}

def md_cell(source: str):
    lines = source.splitlines(keepends=True)
    if lines and lines[-1] == "\n":
        lines.pop()
    return {"cell_type": "markdown", "metadata": {}, "source": lines, "id": str(uuid.uuid4())[:8]}
...
# Cell 1 — Markdown: 元信息
# ═════════════════════════════════════════════════════════════════════
cell1_md = """# AFM Force-Distance Analysis: JJS Suspended COF Film
...
# ── A4 学术报告尺寸标准 ─────────────────────────────────────────────
# 单栏图：8.6 cm 宽；双栏图：17.8 cm 宽（1 inch = 2.54 cm）
...
# 标准处理单元 1: 实验参数汇总
# ═════════════════════════════════════════════════════════════════════
unit1_md = """## 1. Experimental Parameters
...
unit1_code = r'''# ── 1. Experimental Parameters Table ───────────────────────────────
import pandas as pd

params = [
    ("Probe radius $R$", "8", "nm"),
    ("Cantilever stiffness $k_c$", "5.0", "N/m"),
    ("Water surface tension $\\gamma$", "72", "mN/m"),
...
# 标准处理单元 2: 理论力计算与对比
# ═════════════════════════════════════════════════════════════════════
unit2_md = r"""## 2. Classic Force Theory vs. Experiment
...
unit2_code = r'''# ── 2. Theory vs. Experiment Comparison ────────────────────────────
# Physical constants
R_nm = 8.0
d0_nm = 0.3
A_J = 4e-19
...
# ═════════════════════════════════════════════════════════════════════
# 标准处理单元 3: Snap-in 力统计
# ═════════════════════════════════════════════════════════════════════
unit3_md = r"""## 3. Snap-in Force Statistics (All 11 Curves)
...
unit3_code = r'''# ── 3. Snap-in Force Statistics ────────────────────────────────────
rows = []
for d in data:
    rows.append({
...
disp_colors = [COLORS[0] if r["Disp (nm)"] == 454 else
...
# Legend for displacement groups
...
# 标准处理单元 4: 位移依赖性分析
# ═════════════════════════════════════════════════════════════════════
unit4_md = r"""## 4. Snap-in Force vs. Piezo Displacement
...
> **Note:** Equations (1)–(4) from the previous unit define the classic forces; no new formulas are introduced here.
"""

unit4_code = r'''# ── 4. Displacement Dependence ─────────────────────────────────────
...
popt, pcov = curve_fit(linear, disp_sorted, disp_mean)
a_fit, b_fit = popt
...
ax.errorbar(disp_sorted, disp_mean, yerr=disp_std,
            fmt="s", color=COLORS[1], markersize=8, capsize=3, capthick=1.2,
            elinewidth=1, zorder=4, label="Group mean ± SD")

# Fit line
ax.plot(x_fit, y_fit, "--", color=COLORS[4], linewidth=1.2, zorder=2,
        label=f"Linear fit: $F = {a_fit:.3f}d + {b_fit:.1f}$, $R^2 = {r2:.3f}$")
...
# 标准处理单元 5: 不对称性分析
# ═════════════════════════════════════════════════════════════════════
unit5_md = r"""## 5. Drop vs. Rise Slope Asymmetry
...
unit5_code = r'''# ── 5. Asymmetry Analysis ──────────────────────────────────────────
...
bars1 = ax.bar(x - width/2, drop_slopes, width, label="Drop $|k_{drop}|$", color=COLORS[0], edgecolor="black", linewidth=0.3, zorder=3)
bars2 = ax.bar(x + width/2, rise_slopes, width, label="Rise $k_{rise}$", color=COLORS[1], edgecolor="black", linewidth=0.3, zorder=3)
...
# 标准处理单元 6: 能量耗散
# ═════════════════════════════════════════════════════════════════════
unit6_md = r"""## 6. Energy Dissipated in the Transition Zone
...
for d in set(disps):
    ...
    ax.scatter(x_vals, y_vals, c=disp_color_map.get(d, COLORS[4]), s=50, alpha=0.7, zorder=3, edgecolors="white", linewidth=0.5, label=f"{int(d)} nm")

ax.set_xlabel("Piezo Displacement (nm)")
ax.set_ylabel("Dissipated Energy (nN·nm)")
...
# 标准处理单元 7: 典型力曲线
# ═════════════════════════════════════════════════════════════════════
unit7_md = r"""## 7. Representative Force-Distance Curves
...
unit7_code = r'''# ── 7. Representative Force Curves ─────────────────────────────────
...
for idx, (ax, d_idx, lbl, mk) in enumerate(zip(axes, rep_indices, labels_rep, markers)):
    ...
    # Approach (drop) — negative force region
...
# 标准处理单元 8: 结论汇总
# ═════════════════════════════════════════════════════════════════════
unit8_md = r"""## 8. Summary of Key Findings
...
unit8_code = r'''# ── 8. Summary Statistics Table ────────────────────────────────────
summary = {
    "Metric": [
        "Curves analyzed",
        "Mean $|F_{snap}|$",
...
cells = [
    ...
    md_cell(unit1_md),
    code_cell(unit1_code),
    md_cell(unit2_md),
    code_cell(unit2_code),
    md_cell(unit3_md),
    code_cell(unit3_code),
    md_cell(unit4_md),
    code_cell(unit4_code),
    md_cell(unit5_md),
    code_cell(unit5_code),
    md_cell(unit6_md),
    code_cell(unit6_code),
    md_cell(unit7_md),
    code_cell(unit7_code),
    md_cell(unit8_md),
    code_cell(unit8_code),
]

OUT_PATH.parent.mkdir(exist_ok=True)
with open(OUT_PATH, "w", encoding="utf-8") as f:
    json.dump(nb_json(cells), f, indent=1, ensure_ascii=False)

print(f"Generated: {OUT_PATH}")
print(f"Total cells: {len(cells)} ({len(cells)//2} standard units + 2 fixed cells)")
