...
for ax, (key, title) in zip(axes.flat, params):
    ...
    short_labels = [takeoff_rows[i]["File"].replace("linker1-PFPE-OH-", "").replace("linker1-PFPE-OH", "") for i in indices]
...
fig.suptitle(f"PFPE-OH mechanical parameters per curve (N={len(mech_results)})", fontsize=11)
fig.tight_layout()
